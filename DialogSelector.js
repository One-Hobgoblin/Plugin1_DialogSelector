...
DataManager.isMapLoaded = function () {
  let mapIsLoaded =
    tutorialDialogSelectorDatabaseManager_IsMapLoaded.call(this);
  if (mapIsLoaded) {
...
/* Plugin Commands */
var tutorialDialogSelectorGameInterpreter_pluginCommand =
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  if (command === "Tutorial.DialogSelector") {
    ...
    if (
      command.match(
        /Tutorial.DialogSelector[ ]EnableDialogSystem[ ](?:(\w+)|(\d+))/
      )
    ) {
      matches =
        /Tutorial.DialogSelector[ ]EnableDialogSystem[ ](?:(\w+)|(\d+))/.exec(
          command
        ) || [];
      if (matches.length > 1) {
      ...
      command.match(/Tutorial.DialogSelector[ ]GetDialog[ ](\d+)[ ](\d+)/)
    ) {
      matches =
        /Tutorial.DialogSelector[ ]GetDialog[ ](\d+)[ ](\d)/.exec(command) ||
        [];
      if (matches.length > 2) {
    ...
    tutorialDialogSelectorGameInterpreter_pluginCommand.call(
      this,
      command,
      args
    );
...
Game_System.prototype.setDialogVariable = function (mapId, npcId) {
  ...
  if (mapId && mapId > 0 && $gameMap._mapId == mapId) {
      ...
      if (npcId && npcId > 0) {
        ...
        if (event && npc) {
          ...
          for (let dialogObject of dialogObjectList) {
            ...
            let conditions = gameSwitches.filter((swt, idx) =>
              conditionList.includes(idx)
            );
